...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        for conta in ContasMercadoLivre.objects.filter(status='Conectado'):
            ...
            headers={'Authorization':'Bearer '+ conta.access_token, 'X-Format-New': 'true'}
            params = {}
            ...
            if 'results' in dados:
                for result in dados['results']:
                    ...
                    pedido_id = '2000007343943856'
                    pedido_id = '2000007355565694'
                    ...
                    pack_id =  resposta_pedido.json()['pack_id']
                    #if not pack_id:
                    #   continuea
                    shipping_id =  resposta_pedido.json()['shipping']['id']
                    params = {'text':'Oi'}
                    params = {}
                    buyer_id =  resposta_pedido.json()['buyer']['id']
                    params = {"from" : {"user_id": seller['id'],},"to": {"user_id" : buyer_id},"text": "Oi",}
                    params = {"speed": 16, "status":"shipped","tracking_number": 'tnt355',"receiver_id": 355}
                    #params = {"payload": {"service_id": 11,"comment": "despachado","date": "2023-01-16T13:03:51.175-04:00"},"tracking_number": "PAJU355","tracking_url": "http://www.url.test/40886674732","status": "shipped","substatus": "null"}
                    
                    import json
                    #params =json.dumps(params)
                    url_mensagem = "https://api.mercadolibre.com/messages/packs/%s/sellers/%s?tag=post_sale" % (pedido_id, seller['id'])
                    ...
                    url_caminho = 'https://api.mercadolibre.com/shipments/%s/seller_notifications' % shipping_id
                    url_caminho = 'https://api.mercadolibre.com/shipments/%s' % shipping_id
                    #url_caminho = 'https://api.mercadolibre.com/items'
                    params =[{"id": "26379079680","tracking_number": "1234NLUG123"}]
                    params = {"tracking_number": "1234NLUG123","code": "0201"}
                    params = {"speed": "12","status":"shipped","tracking_number": "000000001234","receiver_id": "12345678"}
                    resposta_caminho = requests.put(url=url_caminho, headers=headers, json=params)
                    print(resposta_caminho.text)
                    import pdb;pdb.set_trace()
